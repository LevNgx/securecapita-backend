...
public class HandleException extends ResponseEntityExceptionHandler implements ErrorController {
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException e, HttpHeaders headers, HttpStatusCode status, WebRequest request) {
        ...
        String fieldMsg = fieldErrorList.stream().map(FieldError::getDefaultMessage).collect(Collectors.joining(", "));
        return new ResponseEntity<>(
                HttpResponse.builder()
                        .timeStamp(now().toString())
                        .reason(fieldMsg)
    ...
    public ResponseEntity<HttpResponse> sQLIntegrityConstraintViolationException(SQLIntegrityConstraintViolationException e) {
        return new ResponseEntity<>(
                HttpResponse.builder()
                        .timeStamp(now().toString())
                        .reason(e.getMessage().contains("Duplicate entry") ? "Information already exists" : e.getMessage())
                        .developerMessage(e.getMessage())
                        .status(BAD_REQUEST)
